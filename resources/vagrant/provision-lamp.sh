...
sudo cp /vagrant/resources/vagrant/conf/apache-lamp.conf /etc/apache2/sites-available/
sudo a2ensite apache-lamp
sudo a2enmod rewrite
sudo systemctl reload apache2

# MariaDB - bind to all ip addresses for remote connections
sudo cp /vagrant/resources/vagrant/conf/mariadb-bind-ip.cnf /etc/mysql/mariadb.conf.d/99-bind-all-ip.cnf
